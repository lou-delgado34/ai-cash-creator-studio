...
function CheckoutContent() {
  ...
  return (
    <section className="mx-auto max-w-2xl rounded-3xl border border-white/10 bg-zinc-900 p-8">
      ...
      <h1 className="mt-3 text-4xl font-bold">You selected {selected.name}</h1>

      <p className="mt-4 text-5xl font-bold text-blue-400">{selected.price}</p>
...
export default function CheckoutPage() {
  return (
    <main className="min-h-screen bg-black px-6 py-10 text-white">
      <Suspense fallback={<p className="text-white">Loading checkout...</p>}>
        <CheckoutContent />
      </Suspense>
    </main>
  );
